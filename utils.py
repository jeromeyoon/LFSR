...
def get_image(input_,label_,image_size,is_crop=True):
    ...
    gt = transform(np.expand_dims(label_,axis=-1),randx,randy,image_size)
    return np.concatenate((inputs,gt),axis=2)

def get_image_test(input_,label_,is_crop=True):
    inputs = input_/255.0
    gt = np.expand_dims(label_,axis=-1)
    return np.concatenate((inputs,gt),axis=2)
